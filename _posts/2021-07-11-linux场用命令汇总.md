...
当内存不够用的时候，可以将 buff/cache 释放掉，如果真的不够用 linux 会自动将一部分内存移到 swap 分区（交换分区），交换分区是在磁盘上，磁盘上的读写效率比内存低 10 倍。redis memorycahe 无法添加到交换分区中，只能增大内存

磁盘文件会存放在/dev/sd\* 有多少个分区就有多少个 sd 开头的文件。 磁盘的存储单位为扇区。每一个扇区为 512 字节
